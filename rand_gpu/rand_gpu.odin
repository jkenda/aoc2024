...
foreign rand_gpu {
     ...
     */
    new_rng :: proc(algorithm: Algorithm, n_buffers: c.size_t, buffer_multi: c.size_t) -> ^Rng ---
     ...
     * @param buffer_multi Buffer size multiplier
     */
    new_rng_with_seed :: proc(algorithm: Algorithm, n_buffers: c.size_t, buffer_multi: c.size_t, seed: c.uint64_t) -> ^Rng  ---
    ...
    _rng_put_random :: proc(rng: ^Rng, dst: rawptr, nbytes: c.size_t) ---

    /**
     * @brief Discards u bytes from RNG's buffer.
     */
    rng_discard :: proc(rng: ^Rng, z: c.size_t) ---
     ...
     */
    rng_buffer_misses :: proc(rng: ^Rng) -> c.size_t ---
     ...
     */
    algorithm_name :: proc(algorithm : Algorithm, description: c.bool) -> cstring ---
...
new_rng_default :: proc() -> ^Rng {
    return new_rng_with_seed(.TYCHE, 2, 16, rand.uint64(runtime.default_random_generator()))
}
...
random_generator_default :: proc() -> runtime.Random_Generator {
    return {
        procedure = proc(data: rawptr, mode: runtime.Random_Generator_Mode, p: []byte) {
            ...
            case .Reset:
                // TODO: implement reset in rand_gpu
            case .Query_Info:
...
// random_generator returns a `runtime.Random_Generator` that generates random numbers on the GPU
random_generator :: proc(algorithm: Algorithm, n_buffers: c.size_t, buffer_multi: c.size_t) -> runtime.Random_Generator {
    return {
        procedure = proc(data: rawptr, mode: runtime.Random_Generator_Mode, p: []byte) {
            switch mode {
            case .Read:
        ...
        data = new_rng_with_seed(algorithm, n_buffers, buffer_multi, rand.uint64(runtime.default_random_generator())),
    }
}
